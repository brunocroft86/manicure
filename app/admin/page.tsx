...
export default function AdminDashboard() {
  ...
  // === INÍCIO DO SISTEMA EM TEMPO REAL E NOTIFICAÇÕES ===
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT' && payload.new.client_name !== "🔒 BLOQUEIO") {
            if (Notification.permission === "granted") {
              new Notification("Novo Agendamento! 🎉", {
                body: `${payload.new.client_name} acabou de marcar um horário!`,
              });
            } else {
              alert(`🎉 NOVO AGENDAMENTO!\n\n${payload.new.client_name} acabou de marcar um horário!`);
            }
          }
        }
      )
            ...
            <div className="flex flex-col lg:flex-row gap-6 sm:gap-8">
              ...
              <div className="w-full lg:w-2/3">
                <div className="bg-white p-6 sm:p-8 rounded-[2rem] shadow-sm border border-slate-200/60">
                  ...
                  {/* === O SEGREDO DO LAYOUT COMPACTO ESTÁ AQUI === */}
                  <div className="space-y-3 bg-slate-50/50 p-4 rounded-2xl border border-slate-100"> 
                    {services.length === 0 && <div className="text-center py-12 bg-white rounded-xl border-2 border-dashed border-slate-200"><p className="text-slate-400 font-medium">Nenhum serviço cadastrado ainda.</p></div>}
                    {services.map((service) => (
                      <div key={service.id} className="flex items-center justify-between p-4 rounded-xl border border-slate-200 bg-white shadow-sm hover:border-pink-300 hover:shadow-md transition-all">
                        {/* Lado Esquerdo: Ícone e Texto */}
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-pink-50 flex items-center justify-center text-pink-500 flex-shrink-0">
                          ...
                          <div>
                            <h4 className="text-sm sm:text-base font-bold text-slate-800 leading-tight">{service.name}</h4>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button onClick={() => handleDeleteService(service.id)} className="text-slate-400 hover:text-red-500 bg-slate-50 p-2.5 rounded-lg hover:bg-red-50 active:scale-95 transition-all cursor-pointer border border-slate-100">

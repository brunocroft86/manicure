...
export default function AdminDashboard() {
  ...
  if (authLoading) {
    return <div className="min-h-screen bg-pink-50 flex items-center justify-center text-pink-500 font-bold animate-pulse">Carregando painel...</div>;
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="h-16 sm:h-20 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={handleLogout} className="text-xs text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-full font-bold transition-colors">
                Sair
              </button>
            </div>
          </div>
                ...
                <div className="bg-white p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] shadow-lg shadow-rose-100/40 border border-white lg:sticky lg:top-36">
                  ...
                  <form onSubmit={handleAddService} className="space-y-4">
                    <div>
                      ...
                      <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-3.5 bg-slate-50 text-slate-900 border border-slate-200 rounded-xl focus:ring-2 focus:ring-pink-200 outline-none text-base" placeholder="Ex: Unha de Gel" />
                    </div>
                    <div className="flex gap-3">
                      <div className="w-1/2">
                        <label className="block text-xs sm:text-sm font-semibold text-slate-600 mb-1.5 pl-1">Valor</label>
                        <input type="text" inputMode="numeric" value={price} onChange={handlePriceChange} className="w-full px-4 py-3.5 bg-slate-50 text-slate-900 border border-slate-200 rounded-xl focus:ring-2 focus:ring-pink-200 outline-none text-base" placeholder="R$ 0,00" />
                      </div>
                      <div className="w-1/2">
                        <label className="block text-xs sm:text-sm font-semibold text-slate-600 mb-1.5 pl-1">Minutos</label>
                        <input type="number" inputMode="numeric" value={duration} onChange={(e) => setDuration(e.target.value)} className="w-full px-4 py-3.5 bg-slate-50 text-slate-900 border border-slate-200 rounded-xl focus:ring-2 focus:ring-pink-200 outline-none text-base" placeholder="60" />
                      </div>
                    </div>
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {services.length === 0 && (
                      <div className="text-center py-12 bg-slate-50/50 rounded-2xl border-2 border-dashed border-pink-100">
                        <p className="text-slate-500 font-medium text-sm sm:text-base">Nenhum serviço encontrado.</p>
                      </div>
                    )}
                    {services.map((service) => (
                      <div key={service.id} className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-4 sm:p-5 rounded-2xl border border-slate-100 hover:border-pink-200 shadow-sm transition-all bg-white gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h4 className="text-base sm:text-lg font-bold text-slate-700 leading-tight">{service.name}</h4>
                            <p className="text-xs sm:text-sm text-slate-400 mt-1 font-medium">{service.duration_minutes} min</p>
                          </div>
                        </div>
